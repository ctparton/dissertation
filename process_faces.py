...
def prepare_dir_for_alignment(image_dir):
    ...
    for image in image_dir.iterdir():
        ...
        if len(images) >= folder_size:
            for im in images:
                Path(PROCESS_PATH, Path(f"image_split_{image_dir.name}"), Path(f"{count}")).mkdir(exist_ok=True, parents=True)
                shutil.copy(im, Path(PROCESS_PATH, Path(f"image_split_{image_dir.name}"), Path(f"{count}"), Path(f"{im.name}")))
            images = []
        ...
        if not face_locations[i]:
            ...
            try:
                try:
                    ...
                    loose_crop.save(
                        Path(PROCESS_PATH, Path(images[i].parent.name), Path("very_loose_crop_crop"), Path(images[i].name)))
                ...
                tighter_crop.save(Path(PROCESS_PATH, Path(images[i].parent.name), Path("loose_crop"), Path(images[i].name)))
            except (ValueError, SystemError):
                error_list.append(images[i])
                pil_image = Image.fromarray(resized_faces[i])
                Path(PROCESS_PATH, Path(images[i].parent.name), "retry").mkdir(exist_ok=True, parents=True)
